...
//   props.setIsSignUp(false);
  ...
  useEffect(()=>{
    ...
    const find_tutorId = await axios.get(
      "https://device2api.el.r.appspot.com/tutor/find_tutorId"
    );
  ...
  const submitHandler = async (event) => {
    ...
    //         .post("https://device2api.el.r.appspot.com/client/getPhone", {email:postData.email1})
    // if(result2.data===null){
    //   setIsCorrect(true)
      if (enteredCPassword === enteredPassword) {
        try {
          await axios
            .post("https://device2api.el.r.appspot.com/tutor/tutorSignUp", postData)
            .then((res) => {
