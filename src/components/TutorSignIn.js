...
const SignInFrom = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setIsloading(true);
    try {
      await AuthService.tutorLogin(email, password).then(
        () => {
          // history.push('/tutor_dashboard');
          if (
            JSON.parse(localStorage?.getItem("tutor"))?.saveTutor?.tutStatus === "ex"
          ) {
            ...
            history.replace("/tutor_regForm");
            setIsloading(false);
          //   let tutor_ls = JSON.parse(localStorage.getItem("tutor"));
          //   let temp_tutorObj = {
          //     ...tutor_ls,
          //     reach: "tutor_regform",
          //   };
          //   localStorage.setItem("tutor", temp_tutorObj);
          }
