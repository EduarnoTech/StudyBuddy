...
const UpcomingSessions = (props) => {
 ...
 const count=Math.ceil(((upcomingAr!==undefined && upcomingAr.length!==0) ? upcomingAr.length:props.upcomingSessionUser.length) / PER_PAGE);
 const  _DATA = UsePagination((upcomingAr!==undefined && upcomingAr.length!==0) ? upcomingAr:props.upcomingSessionUser, PER_PAGE);
   ...
   <Header profilePic={props.profilePic} name = {props.values.username} />
    ...
    >
      <Container maxWidth={false}>
        <ProductListToolbar upcomingSessionUser={props.upcomingSessionUser} setUpcomingAr={setUpcomingAr}/>
        <Box sx={{ pt: 3 }}>
          <Grid
            container
            spacing={3}
          >
           ...
           {( _DATA.currentData()!==undefined && _DATA.currentData().length!==0)  ?  _DATA?.currentData().length!==0? _DATA?.currentData().map((product) => (
              <Grid
                item
            ...
            <Grid
            item
            style={{marginLeft:"42%",fontStyle:"italic",fontSize:"1rem"}}
            key={"1"}
            lg={4}
            md={6}
            xs={12}
          >
           ...
           :props.upcomingSessionUser.length!==0? props?.upcomingSessionUser?.map((product) => (
             <Grid
                item
                key={product.sessionId}
                lg={4}
