 ...
 useEffect(()=>{
  ...
  const get_questions = async () => {
    ...
    const getQues = await Axios.post(
      // "https://device6chatapi.el.r.appspot.com/api/tutor/get_questions",
      "https://device6chatapi.el.r.appspot.com/api/tutorweb/get_questions",
      { subjectList: subList }
    );
    ...
    if (getQues?.data?.questionArr?.length !== 0) {
      ...
      if (base64String.length !== 0) {
        ...
        history.replace("/tutor_exam/sheet");
      } else {
        setLoaderVisible(false)
    ...
    <div>
      ...
      >
        <ul class="form-section page-section">
          ...
          <li>
            <div style={{textAlign: "-webkit-center",marginTop:"20px",marginBottom:"20px"}}><span style={{fontSize:"15px"}}>If your subject is not present in this list,Please contact Us</span>
            <span style={{ marginLeft: '20px'}}>
                <Button variant="contained" className='ctaBtn' style={{background: "linear-gradient(127deg, #03D757, #11A1DC)", borderRadius: '35px', fontSize: '15px'}}  endIcon={<WhatsAppIcon style={{color: '#ffff', fontSize: '25px'}} />} href={'https://wa.me/917070505631'} target='_blank'>
                  WhatsApp Now
                </Button>
            </span>
            </div>
